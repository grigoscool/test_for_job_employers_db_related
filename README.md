...
свое:
14. Функционал первого метода представляет собой следующее (для вызова стоит использовать GET запрос):
- Скачивает  данные с удаленного сервера оп ссылке GET https://jsonplaceholder.typicode.com/posts.

- Сохраняет все строки в базу данных. 
- Возвращает в ответе 201 http-код и список cтрок title

15. Функционал второго метода делает следующее (для вызова стоит использовать POST запрос):
 - выгружает из БД пост, отправляет post запрос с ним. 
 - получает новый пост, и записывает его в бд.
16. Функционал третьего метода делает следующее (для вызова стоит использовать POST запрос):
- Передача в метод ВАШЕГО имени и фамилии, а так же ссылки на репозиторий вашего выполненного проекта.
- Подгрузить расшифрованные данные из БД и отправить на удаленный сервер POST
